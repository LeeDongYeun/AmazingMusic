...
/**
 * Purpose: This is a test for the LinkedList Input Requirement:nothing Return:
 * True or False
 */
public class LinkListTest {
	...
	static void LinkedListTester() {
		...
		if (length != ll.getLength() || iniArr.equals(ll.toArray())) {
			...
			System.out.println("LinkedList initial success.");
		int deleteItem = (int) (Math.random() * 10);
		for (; deleteItem >= ll.getLength();)
			deleteItem = (int) (Math.random() * 10);
		int task = -0xFF;
		Object[] obj = ll.toArray();
		task = 0x01;
		// comObj is the object array operated by local array, not transformed
		// from LinkedList
